...
function notifikasi($set = 'out')
{
  if ((isset($_SESSION['notifikasi']) && $_SESSION['notifikasi'] != '')) {
    $r = $_SESSION['notifikasi'];
    if ($r['type'] == 'alert') { ?>
      <div class="alert <?= 'alert-' . $r['status']; ?> alert-dismissible<?= ($set != 'in') ? ' mx-3' : '' ?>">
        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
        <h5><i class="<?= $r['icon']; ?>"></i> <?= $r['head']; ?></h5>
        <?= $r['field']; ?>
      </div>
    <?php } else { ?>
      <div class="callout <?= 'callout-' . $r['status']; ?>">
        <h5><i class="<?= $r['icon']; ?>"></i> <?= $r['head']; ?></h5>
        <p><?= $r['field']; ?></p>
      </div>
<?php }
...
function format_rupiah($angka, $prefix = null)
{
